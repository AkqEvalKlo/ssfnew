...
* Kurzbeschreibung :: Testdriver fuer SSF-Modul SSFCIN0
*
...
* --------------------
*
* Testdriver fuer SSF-Modul SSFCIN0 (CHECKIN)
*
******************************************************************
...
*--------------------------------------------------------------------*
 01          KONSTANTE-FELDER.
     05      K-MODUL             PIC X(08)          VALUE "CINDRV0O".
...
*       "RI" = Recover Index (HFILE -> TAB)
*       "SI" = Save Index (TAB -> HFILE)
     10 RCI-RC                 PIC S9(04) COMP.
*       0    = OK
*       9999 = Programmabbruch - Hauptprogramm muss reagieren
    05  RCI-DATA.
*       Name des Repository-Files (z.b. PFCSIP7R)
     10 RCI-REP-FILE           PIC X(36).
     10 RCI-REP-FILE-LEN       PIC S9(04) COMP.
...
******************************************************************
 ...
 B100-00.
     ...
     MOVE STUP-MOD-CMD       TO LINK-CMD
     MOVE ZERO               TO LINK-RC
     MOVE STUP-MOD-FILE      TO LINK-REP-FILE
     ENTER TAL "String^Laenge" USING  LINK-REP-FILE, 36
                               GIVING LINK-REP-FILE-LEN
                               
**  --> Aufrufen SSFCIN0
     CALL "SSFCIN0M"     USING LINK-REC
     EVALUATE LINK-RC
     
        WHEN   ZERO   CONTINUE
        
        WHEN   9999   DISPLAY " RC 9999 = PRG-ABBRUCH aus SSFCIN0 "
...
**  --> Jetzt den Index pflegen
      PERFORM H100-SAVE-INDEX
     .
 B100-99.
     EXIT.
...
******************************************************************
 ...
 C000-99.
     EXIT.
******************************************************************
* Sicher des Index [FNAME]H aus SSFRARCH
******************************************************************
 H100-SAVE-INDEX SECTION.
 H100-00.
    
     MOVE "SI"               TO RCI-CMD
     MOVE ZERO               TO RCI-RC   
*     MOVE P-SRC-FILE         TO RCI-REP-FILE
     MOVE STUP-MOD-FILE      TO RCI-REP-FILE
     ENTER TAL "String^Laenge" USING  RCI-REP-FILE, 36
                               GIVING RCI-REP-FILE-LEN
                               
**  --> Aufrufen SSFRCI0
     CALL "SSFRCI0M"     USING RCI-REC
     EVALUATE RCI-RC
     
        WHEN   ZERO   CONTINUE
        
        WHEN   9999   DISPLAY " RC 9999 = PRG-ABBRUCH aus SSFRCI0 "
                      SET PRG-ABBRUCH TO TRUE
                      
        WHEN   OTHER  MOVE RCI-RC TO D-NUM4
                      DISPLAY " unbekannter RC: ",
                                D-NUM4,
                              " aus SSFRCI0"
                      DISPLAY " (< ZERO = SQL-Fehler"
